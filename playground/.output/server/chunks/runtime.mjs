...
const _DVs8OX = defineEventHandler(async (event) => {
  ...
  try {
    ...
    const validationResult = await KnownUser.validateRequestByIntegrationConfig(
      ...
      (queueitToken == null ? void 0 : queueitToken.toString()) || "",
      configuration,
      customerId,
      customerSecretKey,
      contextProvider,
...
function isIgnored(event, {
  ...
  const { path } = event;
  const anyPathMatch = ignoreHttpPaths.filter((it) => {
...
function buildContextProvider(event, { cookiePath, body }) {
  ...
  return {
    ...
    getHttpRequest() {
      return {
        ...
        getUserHostAddress() {
          return getRequestIP(event, { xForwardedFor: true }) || event.node.req.socket.remoteAddress;
        },
    ...
    getHttpResponse() {
      return {
        setCookie(cookieName, cookieValue, domain, expiration, isCookieHttpOnly, isCookieSecure) {
          if (domain === "") {
            domain = void 0;
          }
          const expirationDate = new Date(expiration * 1e3);
...
function getAbsoluteRequestUrl(event) {
  const absoluteUrl = getRequestURL(event);
  return absoluteUrl.toString();
}
function getRequestWithoutToken(event) {
...
const _lazy_BC92mi = () => import('./routes/renderer.mjs').then(function (n) { return n.r; });

const handlers = [
  { route: '', handler: _f4b49z, lazy: false, middleware: true, method: undefined },
  { route: '/__nuxt_error', handler: _lazy_BC92mi, lazy: true, middleware: false, method: undefined },
  { route: '', handler: _DVs8OX, lazy: false, middleware: true, method: undefined },
  { route: '/**', handler: _lazy_BC92mi, lazy: true, middleware: false, method: undefined }
];
